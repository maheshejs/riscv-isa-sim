...
//////////////////////////////////////////
/// (M,N) = (1,3) correlating predictor 
//////////////////////////////////////////

#define TABLE_SIZE 16384
#define PRIME 16381
#define NUM_BITS_COUNTER 3

int table[NUM_CPUS][TABLE_SIZE];
uint8_t last_outcomes[NUM_CPUS]; // 1-bit history
...
unsigned int gs_table_hash(uint64_t pc, int bh_vector)
{
    unsigned int hash = pc ^ (pc >> GLOBAL_HISTORY_LENGTH) ^ (pc >> (GLOBAL_HISTORY_LENGTH * 2)) ^ bh_vector;
    ...
    last_pc(1), executions(1), TM(cfg->trigger_count)
{
    ...
    reg_t branch_predictor_id = cfg->branch_predictor_id;
    if (branch_predictor_id == 2)
    {
        branch_predictor = PERCEPTRON;
    }
    else if (branch_predictor_id == 1)
    {
    ...
    switch (branch_predictor) {
        case MN:
            {
            fprintf(stderr, "(m,n) branch predictor\n");
            last_outcomes[cpu] = 0;
        ...
        case GSHARE:
            {
            fprintf(stderr, "gshare branch predictor\n");
        ...
        case PERCEPTRON:
            {
            fprintf(stderr, "perceptron branch predictor\n");
            spec_global_history[cpu] = 0;
            global_history[cpu] = 0;
            perceptron_state_buf_ctr[cpu] = 0;
            for (int i=0; i<NUM_PERCEPTRONS; i++)
            {
                for (int j=0; j<=PERCEPTRON_HISTORY; j++) perceptrons[cpu][i].weights[j] = 0;
            }
            }
            break;
...
static std::string get_branch_name(reg_t fn3)
{
    switch(fn3)
    {
        case 0:
...
processor_t::~processor_t()
{
    ...
    // Branch histogram
    std::vector<std::pair<reg_t, uint64_t>> ordered_histo(branch_histogram.begin(), branch_histogram.end());
    std::sort(ordered_histo.begin(), ordered_histo.end(),
            [](auto& lhs, auto& rhs) { return lhs.second < rhs.second; });

    fprintf(stderr, "\nBranch Histogram size:%zu\n", ordered_histo.size());
    uint64_t branches = 0;
    for (auto it : ordered_histo)
    {
        fprintf(stderr, "%s %" PRIu64 "\n", get_branch_name(it.first).c_str(), it.second);
        branches += it.second;
    }

    std::vector<std::pair<reg_t, uint64_t>> ordered_histo_mp(branch_mp_histogram.begin(), branch_mp_histogram.end());
    std::sort(ordered_histo_mp.begin(), ordered_histo_mp.end(),
            [](auto& lhs, auto& rhs) { return lhs.first < rhs.first; });

    fprintf(stderr, "\nBranch Misprediction Histogram size:%zu\n", ordered_histo_mp.size());
    for (auto it : ordered_histo_mp)
    {
        fprintf(stderr, "%0" PRIx64 " %" PRIu64 " %" PRIu64 "\n", it.first, it.second, 
                it.second + branch_gp_histogram[it.first]);
    }

    fprintf(stderr, "\nBranch Mispredictions: %zu/%zu\n", branch_mispredictions, branches);
...
void processor_t::take_interrupt(reg_t pending_interrupts)
{
    ...
    if (enabled_interrupts == 0) {
        ...
        if (state.v && enabled_interrupts == 0) {
            // VS-ints have least priority and can only be outcome with virt enabled
...
uint8_t processor_t::predict_branch(reg_t pc)
{
    int cpu = 0;
    ...
    switch (branch_predictor) {
        case MN:
            {
            uint32_t hash = pc % PRIME;
            uint8_t prediction = 0;

            uint32_t virtual_hash = hash;
            virtual_hash &= ~1; // Clear LSB to select the row
            virtual_hash |= last_outcomes[cpu]; // Set LSB to history(last_outcome) to select the table
                                                //    one of the 2 virtualized tables
            prediction = table[cpu][virtual_hash] >> (NUM_BITS_COUNTER-1); // Counter MSB has prediction
            return prediction;
            }
            break;
        ...
        case GSHARE:
            {
            int prediction = 1;
            int gs_hash = gs_table_hash(pc, branch_history_vector[cpu]);
...
void processor_t::last_branch_result(reg_t pc, uint8_t outcome)
{
    int cpu = 0;
    ...
    switch (branch_predictor) {
        case MN:
            ...
            uint32_t virtual_hash = hash;
            virtual_hash &= ~1;
            virtual_hash |= last_outcomes[cpu];
            // Branch-free 2-bit saturating counter :)
            uint8_t counter   = (table[cpu][virtual_hash] + (outcome << 1) - 1) &
                ((1 << (NUM_BITS_COUNTER+1)) - 1);
            uint8_t saturated = counter >> NUM_BITS_COUNTER;
            table[cpu][virtual_hash]  = (counter - ((outcome & saturated) << 1) + saturated) &
                ((1 << NUM_BITS_COUNTER) - 1);
            // Update history
            last_outcomes[cpu] = outcome;
            }
            break;
        ...
        case GSHARE:
            ...
            branch_history_vector[cpu] |= outcome;
            }
            break;
        ...
        case PERCEPTRON:
            ...
            global_history[cpu] |= outcome;

            if (u[cpu]->prediction != outcome) spec_global_history[cpu] = global_history[cpu];
            ...
            if (u[cpu]->output > THETA)
                ...
                y = 2;
            if (y == 1 && outcome) return;
            if (y == 0 && !outcome) return;
...
void processor_t::update_branch_mispredictions(insn_bits_t bits, uint8_t prediction, uint8_t outcome)
{
    if (prediction != outcome)
    {
        ++branch_mispredictions;
        ++branch_mp_histogram[bits];
    }
    else
    {
        ++branch_gp_histogram[bits];
    }
}
